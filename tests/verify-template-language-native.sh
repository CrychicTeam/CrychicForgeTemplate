...
remove_verify_sources() {
  ...
  rm -f "$ROOT/src/main/java/org/pickaid/templateverify/NativeLoadHarness.java"
...
run_native_load_harness() {
  ...
  mkdir -p "$ROOT/src/main/java/org/pickaid/templateverify"
  cat > "$ROOT/src/main/java/org/pickaid/templateverify/NativeLoadHarness.java" <<JAVA
package org.pickaid.templateverify;
...
JAVA
  cat > "$HARNESS_DIR/native-load.init.gradle" <<'GRADLE'
gradle.projectsEvaluated {
    rootProject.tasks.register("runNativeLoadHarness", JavaExec) {
        dependsOn(rootProject.tasks.named("classes"))
        mainClass = "org.pickaid.templateverify.NativeLoadHarness"
        classpath = rootProject.sourceSets.main.runtimeClasspath
        javaLauncher = rootProject.javaToolchains.launcherFor {
            languageVersion = JavaLanguageVersion.of(Integer.parseInt(rootProject.ext.templatePlatform.javaVersion))
        }
    }
}
GRADLE
  echo "[verify] expecting success: native load harness"
  (cd "$ROOT" && bash ./gradlew -I "$HARNESS_DIR/native-load.init.gradle" runNativeLoadHarness --quiet >/tmp/pickaid-template-verify-native-load.log 2>&1)
  rm -f "$ROOT/src/main/java/org/pickaid/templateverify/NativeLoadHarness.java"
}
...
if grep -F '2001.6.4-build.120' "$ROOT/project.toml" "$ROOT/README.MD" "$ROOT/README_EN.md" "$ROOT/gradle/template-defaults.toml" >/dev/null; then
  echo "Outdated Forge 1.20.1 KubeJS example must not appear in this branch" >&2
  exit 1
fi
