...
`PickAIDForgeTemplate-1.17.1` is only for `Forge 1.17.1`. This branch is curated for the real 1.17.1 ecosystem: built-in versions, CurseMaven file ids, local helper packs, and publishing tasks all target this branch.
...
- `Forge 1.17.1` with `ModDevGradle LegacyForge`
...
- Built-in local helper packs: `basic`, `appleskin`, `combat`, `curios`
...
`project.toml` already contains commented 1.17.1 examples you can enable and adjust.
...
You can copy it from [`project.local.toml.example`](/Users/gedwen/Documents/programing/MC/PickAIDForgeTemplate-1.17.1/project.local.toml.example).
