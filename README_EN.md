...
`PickAIDForgeTemplate-1.18.2` is only for `Forge 1.18.2`. This branch is curated for the real 1.18.2 ecosystem: built-in versions, CurseMaven file ids, local helper packs, and publishing tasks all target this branch.
...
- `Forge 1.18.2` with `ModDevGradle LegacyForge`
...
The `kubejs` pack is curated for real `1.18.2` development and can be used for local script smoke tests.
...
`project.toml` already contains commented 1.18.2 examples you can enable and adjust.
...
You can copy it from [`project.local.toml.example`](/Users/gedwen/Documents/programing/MC/PickAIDForgeTemplate-1.18.2/project.local.toml.example).
